...
app.post('/coverlogofolio/upload', coverlogofolio.single('file'), (req, res) => {
  ...
  const directoryPath = './adminable/coverlogofolio/';
...
app.post(`/coverthumbnails/upload`, coverthumbnails.single(`file`), (req, res) => {
  const uploadedFile = req.file; 
  const directoryPath = './adminable/coverthumbnails/'; 
...
app.post(`/coveralbumcovers/upload`, coveralbumcovers.single(`file`), (req, res) => {
  const uploadedFile = req.file; 
  const directoryPath = './adminable/coveralbumcovers/'; 
...
app.post(`/coverlivestreamthumbnails/upload`, coverlivestreamthumbnails.single(`file`), (req, res) => {
  const uploadedFile = req.file; 
  const directoryPath = './adminable/coverlivestreamthumbnails/'; 
...
app.post(`/covermanipulatedthumbnails/upload`, covermanipulatedthumbnails.single(`file`), (req, res) => {
  const uploadedFile = req.file; 
  const directoryPath = './adminable/covermanipulatedthumbnails/'; 
...
app.post(`/coverrealisticthumbnails/upload`, coverrealisticthumbnails.single(`file`), (req, res) => {
  const uploadedFile = req.file; 
  const directoryPath = './adminable/coverrealisticthumbnails/'; 
...
app.post(`/covermrbeasttypethumbnails/upload`, covermrbeasttypethumbnails.single(`file`), (req, res) => {
  const uploadedFile = req.file;
  const directoryPath = './adminable/covermrbeasttypethumbnails/';
...
app.post(`/mainlogo/upload`, mainlogo.single(`file`), (req, res) => {
  const uploadedFile = req.file;
  const directoryPath = './adminable/mainlogo/';
...
app.post(`/footerlogo/upload`, footerlogo.single(`file`), (req, res) => {
  const uploadedFile = req.file; 
  const directoryPath = './adminable/footerlogo/'; 
...
const managementid = "648c1963423925b00ef308cf" 
 
app.post('/management/change', async(req,res)=>{

const {h1,p1,p2,p3,p4} = req.body 
  
Management.findById(managementid) 
  .then(textContent => {
    if (textContent) {
      if(h1){
        textContent.h1 = h1 
...
app.get('/management/get', async (req, res) => {
  try {
    const management = await Management.findById(managementid);
    if (!management) {
      return res.status(404).json({ error: 'Management document not found' });
    }
    
    const { h1, p1, p2, p3, p4 } = management;
    res.json({h1, p1, p2, p3, p4});
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
  }
